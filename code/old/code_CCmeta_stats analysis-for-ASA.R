...
# Date Created: Oct 25
#
# Date last modified: Dec 3 2018 - by Andrea
#
# Purpose: Start analysis for ASA presentation
#
# Inputs: td_cc-database-clean
#
# Outputs: stats and numbers
#
# NOtes: working off of the spreadsheet that Gina compiled
# AB continued analysis to look at organic systems only
#
##############################

## for AB computer at work
...
ccweed <- read.csv("td_cc-database-clean.csv", na.strings='.', stringsAsFactors=FALSE)


##LRR columns are characters :/ convert to numeric
ccweed$bioLRR <- as.numeric(as.character(ccweed$bioLRR))
ccweed$denLRR <- as.numeric(as.character(ccweed$denLRR))
ccweed$dcc_bio_kgha <- as.numeric(as.character(ccweed$cc_bio_kgha))
ccweed$yieldLRR <- as.numeric(as.character(ccweed$yieldLRR))



posbio <- subset(ccweed, bioLRR < 0) ##90/141 = 55%
posden <- subset(ccweed, denLRR < 0) ##71/147 = 48%
##many positive RRs in Davis, DeBruin (and throughout studies)
##need to look into how these studies explain this.
...
fit <- lmer(denLRR ~ cc_termMETH2 + (1|study),
            data=ccweed2
            #,weights = Wi add this back in for weights
)
summary(fit)
...
fit <- lmer(denLRR ~ cc_type+ (1|study),
            data=ccweed
            #,weights = Wi add this back in for weights
)
summary(fit)
...
xyplot(1:nrow(ccweed2.o) ~ bioLRR , data = ccweed1.o, 
       cex=1.5, #subset = BioPer< 1000,
       auto.key=TRUE,
       pch=20,
...
ccbio <- subset(ccweed, cc_bio_kgha > 5)
ccweed5 <- ccweed[!ccweed$study ==5, ]


xyplot (bioLRR ~ cc_bio_kgha, dat=ccbio, 
        type=c("p"), 
