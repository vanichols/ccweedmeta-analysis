...
wg_ns <- 
  ...
  ungroup() %>% 
  mutate(modlvl = str_to_title(weed_group),
         resp = recode(resp,
                       bio = "Weed Biomass",
                       den = "Weed Density")) %>% 
  select(-weed_group)
...
fig1a <- 
  fig_all %>%
  mutate(sigfill = ifelse(cihi > 0, "S", "NS"),
  ...
  facet_grid(mod_code2 ~resp, scales = "free", switch = "y") +
  geom_hline(yintercept = 0, linetype = "dotted") +
  
  labs(y = "Log of Response Ratio",
       x = NULL, 
       size = NULL) +
  ...
  scale_fill_manual(values = c(wes[5], "black")) +
  theme_bw() +
...
fig1b  
ggsave("_figs/manu_fig3_yield-weeds-WW.png", width = 5.2, height = 5)
